...
import {
  ...
  TrendingUp, Zap, Eye, RotateCcw, Bell, ChevronRight, ArrowLeft,
  Loader2, Cpu, Repeat, KeyRound, BarChart3
} from "lucide-react";
...
export default function AgentGuardDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "costs" | "flow" | "alerts">("overview");

  useEffect(() => {
    if (!authLoading && !user) {
      navigate("/agentguard/auth");
    }
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className={`text-xs font-mono px-2.5 py-1 rounded-lg ${sc.bg} ${sc.text}`}>
                            {agent.status}
                          </span>
                        </div>
                        {agent.description && (
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div>
                            ...
                            <p className="font-mono text-sm text-foreground">${Number(agent.total_cost).toFixed(2)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Calls</p>
                            <p className="font-mono text-sm text-foreground">{agent.total_api_calls}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Tasks</p>
                            <p className="font-mono text-sm text-foreground">{agent.total_tasks}</p>
                          </div>
                        </div>
                        {agent.framework && (
                          <div className="mt-3 flex items-center gap-2">
                            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-muted/30 text-muted-foreground">
                              {agent.framework}
                            </span>
                          </div>
                        )}
